...
describe('useResourceTabs', () => {
  ...
  it('fetches items using foreignKey defaulting to parentSpec.name_id', async () => {
    ...
    await flushPromises()
    expect(mockList).toHaveBeenCalledWith({ company_id: '42' })
  })
  ...
  it('uses custom foreignKey when provided', async () => {
    ...
    await flushPromises()
    expect(mockList).toHaveBeenCalledWith({ org_id: '7' })
  })

  it('uses custom label when provided', () => {
    vi.mocked(createResourceApi).mockReturnValue({ list: vi.fn().mockResolvedValue({ data: [], meta: {} }) } as any)
  ...
  it('stores fetched items in the tab', async () => {
    const users = [{ id: 1, name: 'Alice', _resource: 'user' }, { id: 2, name: 'Bob', _resource: 'user' }]
    vi.mocked(createResourceApi).mockReturnValue({
  ...
  it('re-fetches when parent ID changes', async () => {
    ...
    parentId.value = 5
    await flushPromises()
    expect(mockList).toHaveBeenCalledWith({ company_id: '5' })
  })
